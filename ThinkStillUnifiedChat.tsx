...
/* Premium reader: clean hierarchy, no RGB splitting on reading text */
const RITUAL_SECTION_RE =
    /^(MECHANISM|HOW TO PLAY|PLAY TIME|GOAL|WHY THIS WORKS|WHY IT WORKS|THE PUZZLE BOX|QUIET QUEST|POCKET ESCAPE ROOM|GAME MOVE|FORMULA FLOW|SAFETY|SAFETY TIPS|WIN|REWARD|THE BOX CLICKS WHEN|SMALL CLEAR SIGNAL|PRESS STOP WHEN|RETURN)\s*[:—-]?\s*(.*)$/i
const RITUAL_STEP_RE =
    /^(?:STEP|MOVE|CLUE|ROUND|PHASE|FIRST MOVE|SECOND MOVE|THIRD MOVE)\s*\d*\s*(?:[·/:—-]|\/\/)?\s*(.*)$/i
    ...
    return (
        ...
        >
            {lines.map((line, index) => {
                if (!line)
                    return <div key={`space-${index}`} style={{ height: 9 }} />
                const section = line.match(RITUAL_SECTION_RE)
                const step = line.match(RITUAL_STEP_RE)
                if (!titleUsed && !section && !step && line.length <= 90) {
                    titleUsed = true
                    return (
                        <h2
                            key={index}
                            style={{
                                margin: "0 0 18px",
                                fontSize: Math.max(24, fontSize + 7),
                                lineHeight: 1.16,
                                fontWeight: 780,
                                letterSpacing: -0.45,
                                color: "#FFFFFF",
                        ...
                        <div key={index} style={{ marginTop: index ? 20 : 0 }}>
                            <div
                                style={{
                                    ...
                                    color: "rgba(143,220,255,0.92)",
                                    marginBottom: rest ? 7 : 0,
                                }}
                            >
                                {label}
                            </div>
                            {rest ? (
                                <div
                                    style={{ color: "rgba(249,251,253,0.96)" }}
                                >
                                    {rest}
                                </div>
                            ) : null}
                        ...
                        >
                            <span
                                style={{
                                    ...
                                    borderRadius: 99,
                                    marginTop: 9,
                                    background: "rgba(143,220,255,0.92)",
                                    boxShadow:
                                        "0 0 12px rgba(143,220,255,0.22)",
                                }}
                            />
                            <span>{line}</span>
                        </div>
...
export default function ThinkStillUnifiedChat(props: any) {
    const {
        ...
        comfortLevel = "two",
        showClose = true,
        showBubbleLabelInResponse = false,
        closeFallbackUrl = "",
