// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v38.4.3 runtime + v58.20.9 FINAL RELEASE)
...
type BubbleRitual = {
    ...
    // v58.20.9 engagement/display fields
    return_key?: string
    returnKey?: string
    feedback_prompt?: string
    feedbackPrompt?: string
    feedback_options?: string[] | string
    feedbackOptions?: string[] | string
    if_better?: string
    if_a_little?: string
    if_not_yet?: string
    safety_tier?: number | string
    ...
    for (let attempt = 0; attempt < FETCH_TIMEOUTS_MS.length; attempt++) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (attempt < FETCH_TIMEOUTS_MS.length - 1 && (res.status === 429 || res.status >= 500)) {
            ...
            const timedOut =
                controller.signal.aborted || String(e?.name || "") === "AbortError"
            lastDiag = timedOut
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    const recoverSection = (label: string) => {
        ...
        const labels =
            "GOAL|RITUAL|STEPS|WIN|RETURN KEY|NEXT SAFE KEY|SUPPORTS|TIPS|MIND BEND|FORMULA FLOW|SAFETY|PAUSE|RETURN|LOCK IT IN"
        const re = new RegExp(
            `(?:^|\n)${label}\s*\n([\s\S]*?)(?=\n(?:${labels})\s*(?:\n|$)|$)`,
            "i"
        )
    ...
    const returnKey = String(
        anyR?.returnKey ??
            ...
            anyR?.["Return Key"] ??
            display?.returnKey ??
            recoverSection("RETURN KEY") ??
            recoverSection("NEXT SAFE KEY") ??
            ""
    ).trim()
    const safetyTier = Number(
        anyR?.safety_tier ?? anyR?.["Safety Tier"] ?? 0
    )
    ...
    if (title && (goal || stepCandidates.length)) {
        const sections = [
            ...
            win ? `WIN\n${win}` : "",
            returnKey
                ? `${safetyTier >= 3 ? "NEXT SAFE KEY" : "RETURN KEY"}\n${returnKey}`
                : "",
            supportsRaw.length
                ? `SUPPORTS\n${supportsRaw
                      .map((x: any) => `• ${String(x).trim()}`)
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    for (; i < raw.length; i++) {
        ...
        if (/^(RETURN KEY|NEXT SAFE KEY)$/i.test(upper)) {
            section = "returnKey"
            continue
        }
        ...
        if (section === "returnKey") {
            parsed.returnKey = parsed.returnKey
                ? `${parsed.returnKey}\n${line}`
                : line
            continue
        }
...
type RitualFeedbackState = "idle" | "refine" | "helped" | "safety"
function GameFeedbackPanel({
    ...
    const feedbackOptions = (Array.isArray(optionsRaw)
        ? optionsRaw
        : String(optionsRaw || "").split("|"))
        .map((x) => String(x || "").trim())
        .filter(Boolean)
    const optionLabel = (index: number, fallback: string) =>
        feedbackOptions[index] || fallback
    ...
    const helper = (
        ...
        >
            ...
            <div style={{ minWidth: 0 }}>
                ...
                >
                    {supportOnly
                        ? prompt || "Stay with real-world support."
                        : state === "helped"
                          ? "Shift locked."
                          : state === "safety"
                            ? "Stop here and use Safety."
                            : state === "refine"
                              ? "What needs a different angle?"
                              : prompt || "Did that shift anything?"}
                </div>
            </div>
        </div>
    )
            ...
            {supportOnly ? (
                <div
                    style={{
                        ...
                        fontSize: 12.5,
                        lineHeight: 1.5,
                        color: "rgba(226,237,243,0.82)",
                    }}
                >
                    Stay with the human/support instruction in the Safety block above.
                    Do not switch into another self-help ritual while safety is uncertain.
                </div>
            ) : state === "safety" ? (
                <div
                    style={{
                ...
                >
                    Stop this ritual here. Use the Safety guidance above. If safety is
                    uncertain, connect with human, professional, medical, or emergency
                    support as instructed there.
                </div>
            ) : state === "helped" ? (
                <div
                    style={{
                ...
                >
                    ...
                    >
                        {ifBetter ||
                            "Keep the ritual that worked. Start fresh only when you need another reset."}
                    </div>
                    <button
                        className="ts-haptic"
                        onClick={onNewIssue}
                ...
                >
                    <button className="ts-haptic" onClick={onHelped} style={action(true)}>
                        {optionLabel(0, "CLEAR SHIFT")}
                    </button>
                    <button className="ts-haptic" onClick={onRetry} style={action(false)}>
                        {optionLabel(1, "PARTLY")}
                    </button>
                    <button className="ts-haptic" onClick={onRefine} style={action(false)}>
                        {optionLabel(2, "NO SHIFT")}
                    </button>
                    <button className="ts-haptic" onClick={onLessSteady} style={action(false)}>
                        {optionLabel(3, "LESS STEADY")}
                    </button>
                </div>
            ) : (
...
export default function ThinkStillUnifiedChat(props: any) {
    ...
    const [currentFeedbackPrompt, setCurrentFeedbackPrompt] = React.useState("")
    const [currentFeedbackOptions, setCurrentFeedbackOptions] = React.useState<string[] | string>("")
    const [currentIfBetter, setCurrentIfBetter] = React.useState("")
    const [currentSafetyTier, setCurrentSafetyTier] = React.useState(1)
    ...
    const preloadOverlayUrls = React.useMemo(() => {
        if (!overlayPreloadEnabled || !overlayTransitionsEnabled || !videoOverlayAllowed)
            return [] as string[]
    ...
    const sendMessage = async (
        ...
        try {
            ...
            if (!out) {
                ...
                setCurrentFeedbackPrompt("")
                setCurrentFeedbackOptions("")
                setCurrentIfBetter("")
                setCurrentSafetyTier(1)
            ...
            const chosenAny = out.ritual as any
            const nextRunKey = `run-${Date.now()}-${makeSeed()}`
            setCurrentText(built.text)
            setCurrentFeedbackPrompt(
                String(chosenAny?.feedbackPrompt ?? chosenAny?.feedback_prompt ?? chosenAny?.["Feedback Prompt"] ?? "").trim()
            )
            setCurrentFeedbackOptions(
                chosenAny?.feedbackOptions ?? chosenAny?.feedback_options ?? chosenAny?.["Feedback Options"] ?? ""
            )
            setCurrentIfBetter(
                String(chosenAny?.if_better ?? chosenAny?.["If Better"] ?? "").trim()
            )
            setCurrentSafetyTier(
                Number(chosenAny?.safety_tier ?? chosenAny?.["Safety Tier"] ?? 1) || 1
            )
        ...
        >
            ...
            <style>{`
                ...
                * { -webkit-font-smoothing: antialiased; text-rendering: geometricPrecision; }
                ...
                @media (max-width: 520px) {
                    ...
                    .ts-feedback-actions { grid-template-columns: repeat(2, minmax(0,1fr)) !important; gap: 6px !important; }
            ...
            <motion.div
                ...
                style={{
                    ...
                    // One optimized console skin: calm at rest, with just enough depth
                    // to keep the game-console feel. The strong FX moment is the bubble transition.
                    background:
                        "radial-gradient(80% 72% at 50% -12%, rgba(111,230,255,0.11), rgba(0,0,0,0) 54%), linear-gradient(160deg, rgba(45,56,68,0.99), rgba(16,22,29,1) 50%, rgba(7,11,15,1))",
            ...
            >
                ...
                <div
                    ...
                    style={{
                        ...
                        background:
                            "radial-gradient(circle at 50% 0%, rgba(46,112,132,0.10), transparent 42%), linear-gradient(180deg, rgba(3,9,13,0.94), rgba(1,5,8,0.99))",
                        overflow: "hidden",
                        display: "flex",
                        flexDirection: "column",
                    }}
                    ...
                    >
                        {tab === "saved" ? (
                            ...
                            <>
                                ...
                                {showOverlayTransition && overlayTransition ? (
                                    <motion.div
                                        ...
                                        >
                                            {overlayTransition.isVideo ? (
                                                <video
                                                    ...
                                                    src={overlayTransition.url}
                                                    autoPlay
                                                    muted
                                                    loop={videoPlayTimeSeconds > 0}
                                                    playsInline
                                                    preload="auto"
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <GameFeedbackPanel
                                                ...
                                                onLessSteady={() =>
                                                    setFeedbackState("safety")
                                                }
                                                onCue={(cue) =>
                                                    requestAnotherRitual(cue)
                                                }
                                                onNewIssue={startNewQuest}
                                                prompt={currentFeedbackPrompt}
                                                optionsRaw={currentFeedbackOptions}
                                                ifBetter={currentIfBetter}
                                                supportOnly={currentSafetyTier >= 3}
                                            />
                                        </div>
                                    </div>
                                )}
