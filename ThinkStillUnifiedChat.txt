// ThinkStill 400 — v37.9 TRUE ONE-OF-ONE WIN LOCKED MASTER
// Generated directly from ThinkStill_400_Rituals_v37_9_TRUE_ONE_OF_ONE_WIN_LOCKED_MASTER(2).xlsx
// Production display: TITLE → GOAL → NUMBERED RITUAL → ONE-OF-ONE WIN → 5 PLAIN SUPPORT MOVES → FORMULA FLOW → SAFETY
// Backend-only: Hook, internal Goal, Game Move, source taxonomy, emotional-distress cue and fallback cue.

// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (FULL v37.9 POWER-UP RENDERER FINAL)
...
function ritualTextDefault(r: BubbleRitual) {
    // v37.9 TRUE ONE-OF-ONE WIN LOCK
    // Render exactly from the attached locked master.
    // Backend-only fields are intentionally excluded.
    const title = String(r?.title ?? r?.name ?? r?.ritual_name ?? "").trim()
    const playTime = String(r?.play_time ?? "").trim()
    const goal = String(r?.goal ?? "").trim()
    const ritualSteps = String(r?.ritual_steps ?? r?.play_the_loop ?? "").trim()
    const win = String(r?.win ?? r?.win_reward ?? "").trim()
    const supports = String(r?.support_stack ?? "").trim()
    const flow = String(r?.formula_flow ?? "").trim()
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    for (; i < raw.length; i++) {
        ...
        if (/^(HOOK|MISSION|GOAL)$/i.test(upper)) {
            section = "promise"
            continue
        }
        if (/^RITUAL$/i.test(upper)) {
            section = "steps"
            continue
        }
        ...
        if (/^(POWER[- ]?UP|POTENCY STACK|SUPPORT MOVES)$/i.test(upper)) {
            section = "powerup"
            continue
        }
