// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (FULL v38.4.3 COMPLETE RITUAL RENDERER FINAL)
...
function buildDisplayTitleless(opts: {
    ...
    const firstSafetyIndex = mainLines.findIndex((line) => isSafetyLabelLine(line))
    const main = (firstSafetyIndex >= 0 ? mainLines.slice(0, firstSafetyIndex) : mainLines)
        .join("\n")
        .trim()
    ...
    const safetyLine = (safeLines.length
        ? safeLines
        : [
              "PAUSE",
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    for (; i < raw.length; i++) {
        ...
        if (isSafetyLabelLine(line) || /^PAUSE$/i.test(upper) || /^Safety\s*:/i.test(line)) {
            section = "safety"
            const inline = isSafetyLabelLine(line) ? "" : line.replace(/^Safety\s*:\s*/i, "").trim()
    ...
    const pauseIndex = parsed.safety.findIndex((x) => /^PAUSE$/i.test(String(x || "").trim()))
    ...
    return (
        ...
        >
            ...
            >
                ...
                {ritual.safety.filter((line) => !isSafetyLabelLine(line)).length ? (
                    <div
                        style={{
                            marginTop: 14,
                            padding: "15px 16px",
...
function GameFeedbackPanel({
    ...
    if (state === "helped") {
        return (
            ...
            >
                ...
                >
                    Keep the one that worked. You do not need another ritual right now.
                </div>
                <div
                    style={{
                        marginTop: 8,
                ...
                >
                    Come back with a new signal whenever something else needs a reset.
                </div>
                <button
                    className="ts-haptic"
                    onClick={onNewIssue}
                    style={{ ...action(true), marginTop: 14, textAlign: "center" }}
                >
                    Start a new quest →
                </button>
            </motion.div>
    ...
    return (
        ...
        >
            ...
            >
                If it missed, that is useful information — change the angle, not the goal.
            </div>
            ...
            {state === "idle" ? (
                ...
                <div style={{ marginTop: 14 }}>
                    ...
                    >
                        <button className="ts-haptic" style={cue()} onClick={() => onCue("My thoughts are still looping and I need a different mental angle.")}>Thoughts looping</button>
                        <button className="ts-haptic" style={cue()} onClick={() => onCue("My body still feels activated and I need a more body-focused reset.")}>Body activated</button>
                        <button className="ts-haptic" style={cue()} onClick={() => onCue("The urge is still strong and I need help creating choice before acting.")}>Urge still strong</button>
                        <button className="ts-haptic" style={cue()} onClick={() => onCue("The emotion still feels heavy and I need a gentler different approach.")}>Emotion still heavy</button>
                        <button className="ts-haptic" style={cue()} onClick={() => onCue("I need something more practical and action-focused, not more analysis.")}>Need action</button>
                    </div>
                    <button
                        className="ts-haptic"
                        onClick={onRetry}
                        style={{ ...action(true), width: "100%", marginTop: 12, textAlign: "center" }}
                    >
                    ...
                    {round >= 3 ? (
                        ...
                        >
                            Three rounds and still no fit? Reword what is happening now, or take a break before trying again.
                        </div>
                    ) : null}
                </div>
            )}
...
export default function ThinkStillUnifiedChat(props: any) {
    ...
    const [feedbackState, setFeedbackState] = React.useState<RitualFeedbackState>("idle")
                                        ...
                                        >
                                            ...
                                            <div style={{ maxWidth: 680, margin: "0 auto" }}>
                                                <GameFeedbackPanel
                                                    ...
                                                    state={feedbackState}
                                                    uiFontFamily={uiFontFamily}
                                                    onHelped={() => setFeedbackState("helped")}
                                                    onRefine={() => setFeedbackState("refine")}
                                                    onRetry={() => requestAnotherRitual()}
                                                    onCue={(cue) => requestAnotherRitual(cue)}
