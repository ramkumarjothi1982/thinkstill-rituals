// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v38.8.2 v13 GITHUB FINAL — NO INITIAL PROMPT)
// ✅ v38.8.2 v13 GITHUB FINAL: blank ritual stage until the user enters an issue; no orb / instruction / subtitle
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    // v38.8.2 / v13: the locked workbook's definitive full ritual is the source of truth.
...
function detectHighRiskMessage(message: string) {
    ...
    return /(kill myself|hurt myself|harm myself|end my life|want to die|suicid(?:e|al)|self[-\s]?harm|kill someone|hurt someone|harm someone|medical emergency|overdose|seizure|chest pain|can['’]?t tell what['’]?s real|cannot tell what is real|hearing voices)/i.test(t)
}
...
function buildDisplayTitleless(opts: {
    ...
    // v38.8.2 / v13: preserve the locked v13 ritual exactly. The workbook already contains
...
function pickNextRitualByThinkingError(
    ...
    // safety-scoped pool rather than escaping a high-risk SUPPORT FIRST gate.
    const intentAllowed =
        intent === "neutral"
            ? safetyScopedPool
            : safetyScopedPool.filter((x) =>
                  intentGateAllows(intent, x.ritual)
              )
    const workingPool = intentAllowed.length ? intentAllowed : safetyScopedPool
            ...
            >
                <motion.span
                    ...
                    style={{
                        ...
                        boxShadow: isMind
                            ? `0 0 8px ${cardAccent}, 0 0 24px ${p.glow2}, 0 0 42px ${p.glow2}`
                            : isWin || isPlay
                              ? `0 0 7px ${cardAccent}, 0 0 20px ${isPlay ? p.glow : p.glow2}`
                              : `0 0 5px ${cardAccent}, 0 0 13px ${p.glow}`,
                    }}
                />
                ...
                <motion.section
                    ...
                    style={{
                        ...
                        boxShadow: isPlay
                            ? `0 18px 40px rgba(0,0,0,.31), 0 7px 23px ${p.shadow}, 0 0 30px ${p.glow}, inset 0 1px 0 rgba(255,255,255,.115), inset 0 -1px 0 rgba(0,0,0,.44), inset 14px 0 28px ${p.glow}`
                            : isMind
                              ? `0 18px 42px rgba(0,0,0,.32), 0 7px 24px ${p.shadow}, 0 0 32px ${p.glow2}, inset 0 1px 0 rgba(255,255,255,.105), inset 0 -1px 0 rgba(0,0,0,.44), inset 13px 0 26px ${p.glow2}`
                              : isWin
                                ? `0 15px 34px rgba(0,0,0,.27), 0 5px 18px ${p.shadow}, 0 0 22px ${p.glow2}, inset 0 1px 0 rgba(255,255,255,.085), inset 0 -1px 0 rgba(0,0,0,.40)`
                                : `0 13px 30px rgba(0,0,0,.24), 0 4px 15px ${p.shadow}, inset 0 1px 0 rgba(255,255,255,.078), inset 0 -1px 0 rgba(0,0,0,.36)`,
                ...
                >
                    <div
                        ...
                        style={{
                            ...
                            opacity: isPlay ? 0.82 : isPayoff ? 0.62 : 0.34,
                            filter: "blur(5px)",
                            pointerEvents: "none",
                        }}
                    />
        ...
        <>
            <style>{`
                ...
                @keyframes tsConsoleBreath {
                    0%,100% { opacity:.34; filter:blur(30px); transform:scale(.98); }
                    50% { opacity:.52; filter:blur(36px); transform:scale(1.035); }
                }
                @keyframes tsPlayPulse {
                    0%,100% { box-shadow:0 18px 38px rgba(0,0,0,.30),0 7px 22px ${p.shadow},0 0 22px ${p.glow},inset 0 1px 0 rgba(255,255,255,.075),inset 0 -1px 0 rgba(0,0,0,.42); }
                    50% { box-shadow:0 20px 42px rgba(0,0,0,.32),0 8px 25px ${p.shadow},0 0 38px ${p.glow},inset 0 1px 0 rgba(255,255,255,.085),inset 0 -1px 0 rgba(0,0,0,.42); }
                }
                .ts-depth-ritual {
                    position:relative;
                    isolation:isolate;
                ...
                .ts-depth-path::before {
                    ...
                    box-shadow:0 0 10px ${p.glow}, 0 0 20px ${p.glow2};
                    z-index:0;
                    pointer-events:none;
                }
                .ts-depth-path::after {
                    ...
                    filter:blur(1px);
                    box-shadow:0 0 14px ${p.glow}, 0 0 24px ${p.glow2};
                    opacity:.72;
                    z-index:1;
                    pointer-events:none;
                ...
                .ts-flow-dot::before {
                    ...
                    box-shadow:0 0 16px ${p.glow}, inset 0 1px 0 rgba(255,255,255,.05);
                    opacity:.56;
                    pointer-events:none;
                    z-index:-1;
                }
                ...
                .ts-flow-card-play::before { opacity:.88; box-shadow:0 0 18px ${p.glow}; }
                .ts-flow-card-mind::before { background:linear-gradient(180deg, transparent, ${p.accent2}, transparent); opacity:.82; box-shadow:0 0 18px ${p.glow2}; }
                .ts-flow-card-play {
            ...
            <motion.article
                ...
                style={{
                    ...
                    borderRadius: 32,
                    background: `
                        radial-gradient(118% 78% at 50% -10%, ${p.glow2}, transparent 55%),
                        radial-gradient(90% 54% at 50% 2%, ${p.wash}, transparent 66%),
                        radial-gradient(74% 55% at 100% 100%, ${p.glow2}, transparent 78%),
                        linear-gradient(180deg, rgba(15,29,38,.994), rgba(4,9,14,.999))
                    `,
                    border: `1px solid ${p.line}`,
                    boxShadow: `
                        0 48px 120px rgba(0,0,0,.46),
                        0 20px 44px rgba(0,0,0,.31),
                        0 10px 32px ${p.shadow},
                        0 0 52px ${p.glow2},
...
export default function ThinkStillUnifiedChat(props: any) {
    ...
    const sendMessage = async (
        ...
        try {
            ...
            setRunKey(nextRunKey)
            commitRitualToHistory(
                { text: built.text, sig: built.sig, bubble: actualBubbleSlug },
                isRetry
            )
        ...
        >
            ...
            <style>{`
                ...
                * { -webkit-font-smoothing: antialiased; text-rendering: geometricPrecision; }
                ...
                .ts-console-shell::before {
                    ...
                    pointer-events: none;
                    z-index: 0;
                    background: radial-gradient(ellipse at 50% 0%, rgba(109,232,255,.12), rgba(109,232,255,0) 68%);
                    filter: blur(20px);
                    animation: tsShellAura 11.5s ease-in-out infinite;
                }
                .ts-console-shell::after {
                    ...
                    opacity: .16;
                    background-image:
                        linear-gradient(rgba(150,230,255,.018) 1px, transparent 1px),
                        linear-gradient(90deg, rgba(150,230,255,.012) 1px, transparent 1px);
                    background-size: 34px 34px;
                    mask-image: linear-gradient(to bottom, rgba(0,0,0,.7), transparent 62%);
                }
                @keyframes tsShellAura {
                    0%,100% { opacity:.46; transform:translate3d(-.7%,0,0) scale(.992); }
                    50% { opacity:.72; transform:translate3d(.7%,1%,0) scale(1.018); }
                }
                @keyframes tsEnterBreath {
                    0%,100% { filter:brightness(1) saturate(1); box-shadow:0 0 18px rgba(102,235,255,.12),0 8px 22px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.72); }
                    50% { filter:brightness(1.025) saturate(1.025); box-shadow:0 0 28px rgba(102,235,255,.19),0 9px 24px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.78); }
                }
                ...
                .ts-console-screen::before {
                    ...
                    box-shadow:0 0 18px rgba(104,227,255,.22);
                    animation:tsScreenTopline 10.5s ease-in-out infinite;
                }
                ...
                .ts-console-input:focus {
                    ...
                    box-shadow:inset 0 1px 0 rgba(255,255,255,.065), 0 0 0 3px rgba(97,224,255,.055), 0 0 28px rgba(92,225,255,.075), 0 9px 30px rgba(0,0,0,.22) !important;
                }
            ...
            <motion.div
                ...
                style={{
                    ...
                    overflow: "hidden",
                    position: "relative",
                    border: "1px solid rgba(165,226,244,0.16)",
                    // v38.7.0 magnetic-glass shell: premium depth without adding control clutter.
                    background:
                        "radial-gradient(82% 70% at 50% -16%, rgba(88,225,255,0.145), rgba(0,0,0,0) 58%), radial-gradient(56% 50% at 0% 100%, rgba(174,110,255,0.052), transparent 72%), radial-gradient(44% 40% at 100% 78%, rgba(98,255,210,0.035), transparent 76%), linear-gradient(160deg, rgba(31,47,59,0.998), rgba(11,19,27,1) 52%, rgba(4,8,12,1))",
                    backdropFilter: performanceMode ? "none" : "blur(15px)",
                    display: "flex",
                    flexDirection: "column",
                    animation: performanceMode
                        ? "none"
                        : "tsFrameGlow 6.2s ease-in-out infinite",
                    boxShadow:
                        "0 38px 126px rgba(0,0,0,0.70), 0 0 66px rgba(82,218,255,0.09), inset 0 1px 0 rgba(255,255,255,0.07), inset 0 -1px 0 rgba(0,0,0,.38)",
                }}
            >
                ...
                <div
                    ...
                    style={{
                        ...
                        background:
                            "radial-gradient(44% 130% at 50% 0%, rgba(88,225,255,.065), transparent 70%), linear-gradient(180deg, rgba(255,255,255,0.024), rgba(255,255,255,0.004))",
                    ...
                    >
                        <span
                            ...
                            style={{
                                ...
                                textShadow: "0 0 18px rgba(111,226,255,.20), 0 8px 24px rgba(0,0,0,.28)",
                                whiteSpace: "nowrap",
                            }}
                        >
                            {title} Reset Console
                ...
                <div
                    ...
                    style={{
                        ...
                        marginBottom: 7,
                        borderRadius: 26,
                        border: "1px solid rgba(136,225,247,0.16)",
                        background:
                            "radial-gradient(94% 70% at 50% 0%, rgba(56,164,190,0.105), transparent 56%), radial-gradient(58% 48% at 100% 100%, rgba(142,104,255,.028), transparent 74%), linear-gradient(180deg, rgba(3,9,13,0.975), rgba(1,5,8,0.998))",
                        boxShadow:
                            "inset 0 1px 0 rgba(255,255,255,0.032), 0 20px 54px rgba(0,0,0,0.31), 0 0 42px rgba(77,214,255,0.045)",
                        overflow: "hidden",
                        display: "flex",
                        flexDirection: "column",
                    }}
