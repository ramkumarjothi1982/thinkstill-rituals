// ThinkStill 400 — TRUE ONE-OF-ONE PRODUCT-READY LOCKED MASTER
// Generated from ThinkStill_400_Rituals_TRUE_ONE_OF_ONE_PRODUCT_READY_LOCKED_MASTER(2).xlsx
// Consumer rendering uses the Friend Ritual body and never renders internal Hook / Goal / Game Move.
// Routing uses the balanced Release Routing fields.

// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (TRUE ONE-OF-ONE PRODUCT-READY LOCKED)
...
type FlatManifestFile = {
    ...
    // TRUE ONE-OF-ONE FRIEND EDITION fields
    emotional_distress_cue?: string
    friend_ritual?: string
    if_not_helpful?: string
    breath_support_friend?: string
    mindfulness_support_friend?: string
    positive_psychology_friend?: string
    contemplative_support_friend?: string
    friend_echo_affirmation?: string
    optional_friend_support_stack?: string
    goal_internal?: string
    game_move_internal?: string
    release_routing_trigger?: string
    secondary_routing_trigger?: string
    release_routing_pattern?: string
...
function ritualTextDefault(r: BubbleRitual) {
    // TRUE ONE-OF-ONE LOCKED FRIEND EDITION
    // Internal Hook / Goal / Game Move are never rendered.
    // The attached master intentionally carries the consumer goal/context inside friend_ritual.
    const distress = String(r?.emotional_distress_cue ?? "").trim()
    const title = String(r?.title ?? r?.name ?? r?.ritual_name ?? "").trim()
    const playTime = String(r?.play_time ?? "").trim()

    const rawFriend = String(r?.friend_ritual ?? r?.play_the_loop ?? "").trim()
    const friendLines = normalizeText(rawFriend)
        .split("\n")
        .filter((line) => !/^\s*THINKSTILL\s*$/i.test(line))
        .join("\n")
        .trim()

    const supports = [
        ["🌬 Breath", String(r?.breath_support_friend ?? r?.breathwork_support ?? "").trim()],
        ["◉ Mindfulness", String(r?.mindfulness_support_friend ?? r?.mindfulness_support ?? "").trim()],
        ["✦ Positive Psychology", String(r?.positive_psychology_friend ?? r?.positive_psychology_support ?? "").trim()],
        ["◇ Contemplative", String(r?.contemplative_support_friend ?? r?.spirituality_support ?? "").trim()],
        ["↗ Friend Echo", String(r?.friend_echo_affirmation ?? r?.affirmation_support ?? "").trim()],
    ].filter(([, value]) => Boolean(value))

    const supportSection = supports.length
        ? `FRIEND SUPPORT\n${supports
...
function buildDisplayTitleless(opts: {
    ...
    // Keep the ritual focused. The user already knows what they typed, so the
    // challenge, routing labels and return cue are not repeated in the card.
    const combined = [comfort, main, safetyLine].filter(Boolean).join("\n\n")
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    for (; i < raw.length; i++) {
        ...
        if (/^(POWER[- ]?UP|POTENCY STACK|SUPPORT STACK|FRIEND SUPPORT)$/i.test(upper)) {
            section = "powerup"
            continue
        }
        if (/^YOU WIN WHEN$/i.test(upper)) {
            section = "win"
            continue
        }
        if (/^(LOCK IT IN|TRY ANOTHER RITUAL)$/i.test(upper)) {
            section = "lock"
            continue
        }
        ...
        if (/^(SAFETY|PAUSE)$/i.test(upper) || /^Safety\s*:/i.test(line)) {
            section = "safety"
            const inline = line.replace(/^Safety\s*:\s*/i, "").trim()
            if (inline) parsed.safety.push(inline)
            continue
        }
        if (/^FINISH MOVE\s*[—:-]?/i.test(line)) {
            parsed.finish = line.replace(/^FINISH MOVE\s*[—:-]?\s*/i, "")
            section = ""
            continue
        }
        ...
        >
            ...
            >
                <header style={{ textAlign: "center", marginBottom: 28 }}>
                    ...
                    >
                        {ritual.playTime} · Emotional distress ritual
                    </div>
                    <h1
                        style={{
                            margin: 0,
                ...
                {ritual.finish ? (
                    ...
                    >
                        <strong style={{ color: "#FFFFFF" }}>Finish:</strong>{" "}
                        {ritual.finish}
                    </div>
                ) : null}
