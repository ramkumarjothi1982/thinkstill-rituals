// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v38.4.3 + v58.19 GAME CONSOLE SKIN FINAL — OVERLAY VIDEO VISIBLE PATCH)
// 🎮 FULL CONSOLE SKIN: hardware bezel + HUD + physical controls + mission screen
// ✅ Engine/routing/no-replay/safety behavior preserved; visual/UI layer upgraded
// ✅ Overlay-video visibility patch: defaults to FX, honours allow-video, renders video inside chat screen
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    const stepsRaw =
        typeof (r as any)?.steps === "string" && String((r as any).steps).trim()
            ...
            : Array.isArray((r as any)?.steps)
              ? (r as any).steps
              : Array.isArray((r as any)?.stepsArray)
                ? (r as any).stepsArray.map((x: any, index: number) =>
                      /^\s*\d+[.)]\s+/.test(String(x || ""))
                          ? String(x || "").trim()
                          : `${index + 1}. ${String(x || "").trim()}`
                  )
                : []
    ...
    const mindBend = String(
        (r as any)?.mindBend ?? (r as any)?.mind_bend ?? ""
    ).trim()
    const flowRaw = (r as any)?.formulaFlow ?? (r as any)?.formula_flow ?? ""
...
function inputStyle(
    ...
    return {
        ...
        background: "linear-gradient(180deg, rgba(4,9,14,0.96), rgba(10,17,24,0.92))",
        color: textCol,
        outline: "none",
        fontWeight: 820,
        fontSize: 15,
        letterSpacing: 0.15,
        backdropFilter: perf ? "none" : "blur(10px)",
        boxShadow: "inset 0 2px 9px rgba(0,0,0,0.70), inset 0 0 0 1px rgba(255,255,255,0.035), 0 1px 0 rgba(255,255,255,0.07)",
        fontFamily,
    }
}
function pillBtn(
...
function topIconBtn(perf: boolean, fontFamily: string): React.CSSProperties {
    return {
        ...
        background: "linear-gradient(180deg, rgba(64,75,87,0.98), rgba(24,31,40,0.99))",
        color: "rgba(248,252,255,0.94)",
        fontSize: 20,
        cursor: "pointer",
        backdropFilter: perf ? "none" : "blur(8px)",
        boxShadow: "inset 0 2px 0 rgba(255,255,255,0.12), inset 0 -3px 5px rgba(0,0,0,0.44), 0 4px 0 rgba(5,8,12,0.96)",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        userSelect: "none",
...
function GameFeedbackPanel({
    ...
    const shell: React.CSSProperties = {
        ...
        padding: "18px",
        borderRadius: 15,
        border: "1px solid rgba(116,228,255,0.22)",
        background:
            "linear-gradient(180deg, rgba(14,25,32,0.88), rgba(5,11,16,0.92))",
        boxShadow:
            "0 12px 34px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.055), 0 0 0 4px rgba(0,0,0,0.16)",
        fontFamily: uiFontFamily,
    }
...
export default function ThinkStillUnifiedChat(props: any) {
    ...
    // If the Framer control explicitly allows overlay video, honour it even
    // when the conservative low-power heuristic fires. Turning the control
    // off still restores the guarded behaviour.
    const videoOverlayAllowed =
        !!allowOverlayVideoInPerf || (!lowPower && !performanceMode)
        ...
        >
            <style>{`
                ...
                * { -webkit-font-smoothing: antialiased; text-rendering: geometricPrecision; }
                ...
                .ts-console-shell::after {
                    ...
                    width: 5px;
                    border-radius: 999px;
                    background: linear-gradient(180deg, rgba(107,230,255,.10), rgba(255,255,255,.03), rgba(247,96,206,.08));
                    box-shadow: 0 0 18px rgba(92,219,255,.08);
                    pointer-events: none;
                    z-index: 3;
                }
                .ts-console-shell::before { left: 5px; }
                .ts-console-shell::after { right: 5px; }

                .ts-console-screen {
                    box-shadow:
                        inset 0 0 0 1px rgba(255,255,255,.035),
                        inset 0 0 34px rgba(0,0,0,.60),
                        0 0 0 7px rgba(5,9,13,.60),
                        0 0 0 8px rgba(255,255,255,.055);
                }
                .ts-console-screen::after {
                    content: "";
                    position: absolute;
                    inset: 0;
                    pointer-events: none;
                ...
                @media (max-width: 760px) {
                    .ts-console-shell { border-radius: 24px !important; }
                    .ts-console-shell::before, .ts-console-shell::after { display: none; }
                    .ts-console-topbar { grid-template-columns: 44px minmax(0,1fr) !important; gap: 8px !important; }
                    .ts-console-toggles { grid-column: 1 / -1; justify-content: center !important; flex-wrap: nowrap !important; width: 100%; }
                    .ts-console-brand { font-size: 15px !important; padding: 0 12px !important; }
                    .ts-console-screen { margin-left: 7px !important; margin-right: 7px !important; border-radius: 20px !important; }
                    .ts-console-header { flex-wrap: wrap; }
                    .ts-console-inputrow { gap: 8px !important; }
                    .ts-console-inputrow input { min-width: 0; }
                    .ts-console-inputrow button { flex: 0 0 auto; }
                }

                @media (max-width: 520px) {
                    .ts-console-inputrow { display: grid !important; grid-template-columns: 1fr 48px 48px 88px; gap: 7px !important; }
                    .ts-console-inputrow input { grid-column: 1 / -1; width: 100%; }
                    .ts-console-inputrow button { width: 100% !important; min-width: 0 !important; }
                    .ts-console-brand .ts-console-subtitle { display: none; }
                    .ts-console-header-actions { width: 100%; justify-content: flex-start !important; }
                }
            `}</style>
            ...
            <motion.div
                ...
                style={{
                    ...
                    overflow: "hidden",
                    position: "relative",
                    border: "2px solid rgba(178,206,222,0.20)",
                    background:
                        visualMode === "immersion"
                    ...
                    boxShadow: performanceMode
                        ? "0 34px 110px rgba(0,0,0,0.76), inset 0 2px 0 rgba(255,255,255,0.08), inset 0 -12px 28px rgba(0,0,0,0.30)"
                        : undefined,
                }}
            >
                {showOverlay && !overlayIsVideo ? (
                    <div
                        style={{
                            position: "absolute",
                            inset: 0,
                ...
                >
                    ...
                    <div
                        ...
                        style={{
                            ...
                            gap: 12,
                            padding: "0 16px",
                            background: "linear-gradient(180deg, rgba(15,24,31,0.98), rgba(7,13,18,0.98))",
                            border: "1px solid rgba(116,227,255,0.24)",
                            color: textCol,
                            boxShadow: "inset 0 1px 0 rgba(255,255,255,0.07), inset 0 -7px 18px rgba(0,0,0,0.22), 0 0 18px rgba(79,208,244,0.07)",
                            overflow: "hidden",
                            whiteSpace: "nowrap",
                    ...
                    >
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
                            <span aria-hidden="true" style={{ width: 8, height: 8, borderRadius: "50%", background: "#79F6B2", boxShadow: "0 0 10px rgba(121,246,178,0.72)" }} />
                            <span style={{ opacity: 0.98, fontWeight: 950, letterSpacing: 0.15 }}>{title}</span>
                        </span>
                        <span
                            style={{
                                position: "absolute",
                ...
                <div
                    ...
                    style={{
                        ...
                        marginBottom: 12,
                        borderRadius: 24,
                        border: "1px solid rgba(124,224,247,0.18)",
                        background:
                            visualMode === "immersion"
                                ...
                                : "linear-gradient(180deg, rgba(3,9,13,0.90), rgba(1,5,8,0.97))",
                        overflow: "hidden",
                        display: "flex",
                        flexDirection: "column",
                    }}
                >
                    {/* VIDEO OVERLAY — visible inside the actual chat screen */}
                    {showOverlay && overlayIsVideo ? (
                        <div
                            aria-hidden="true"
                            style={{
                    ...
                    >
                        ...
                        >
                            <span style={{ opacity: 0.72, fontWeight: 760 }}>
                                {tab === "saved" ? "WIN VAULT" : "LIVE QUEST"}
                            </span>
                    ...
                    <div
                        style={{
                            ...
                            zIndex: 30,
                            overflow: "hidden",
                            border: "2px solid rgba(160,233,250,0.28)",
                            background: orbBg(activeBubble),
                            boxShadow: performanceMode
                                ? "0 0 0 5px rgba(4,9,13,0.76), 0 0 24px rgba(140,220,255,0.14)"
                                : "0 0 0 5px rgba(4,9,13,0.76), 0 0 46px rgba(140,220,255,0.24), 0 0 120px rgba(90,190,255,0.10)",
                            pointerEvents: "none",
                            animation: performanceMode
                                ? "none"
                        ...
                        {tab === "saved" ? (
                            <div style={{ display: "grid", gap: 10 }}>
                                {!saved.length ? (
                                    ...
                                    >
                                        ◇ WIN VAULT EMPTY. Clear a quest, then press “SAVE WIN”.
                                    </div>
                                ) : (
                                    saved.map((s) => (
                                        <div
                ...
                >
                    ...
                    >
                        <input
                            ...
                            placeholder="Enter what’s happening right now…"
                            style={inputStyle(
                                textCol,
                                uiFontFamily,
                                performanceMode
