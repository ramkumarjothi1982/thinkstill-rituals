// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v38.4.7 EXACT RITUAL FORMAT + v7 SAFETY + MEDIA PLAYER SIDE-NAV RELEASE)
// ✅ v38.4.7 EXACT FORMAT: renders GAME / MISSION / ONE RULE / PLAY / TWIST / WIN / MIND BEND / SAFETY exactly
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    // v38.4.7 / v7: the workbook's definitive full ritual is the source of truth.
    // Do NOT convert MISSION / ONE RULE / PLAY / TWIST into GOAL + numbered STEPS.
    ...
    // v38.4.6 legacy game-schema bridge. The v6 GitHub library stores the
...
function buildDisplayTitleless(opts: {
    ...
    // v38.4.7: preserve the v7 ritual exactly. The workbook already contains
...
function PremiumRitualText({
    ...
    const sections = [
        ...
        ["ONE RULE", ritual.rule],
        ["PLAY", ritual.play],
        ["TWIST", ritual.twist],
        ["WIN", ritual.win],
        ["MIND BEND", ritual.mindBend],
        ["SAFETY", ritual.safety],
                ...
                {ritual.playTime ? (
                    <div
                        style={{
                            ...
                            border: "1px solid rgba(143,220,255,0.08)",
                    ...
                    <section key={label}>
                        <div
                            style={{
                                ...
                                color:
                                    label === "SAFETY"
                                        ? "rgba(255,205,154,0.98)"
                                        : "rgba(169,229,255,0.96)",
                            }}
                        >
                            {label}
                        </div>
                        <div
                            style={{
                                fontSize:
                                    label === "GAME"
                                        ? Math.max(fontSize + 2, 18)
                                        : fontSize,
                                lineHeight: 1.52,
                                fontWeight: label === "GAME" ? 780 : 520,
                                letterSpacing: label === "GAME" ? -0.2 : 0,
                                color:
                                    label === "SAFETY"
                                        ? "rgba(255,238,220,0.95)"
                                        : "#FFFFFF",
                                whiteSpace: "pre-wrap",
                            }}
                        >
                ...
                {ritual.supportFirst ? (
                    <section
                        style={{
                            padding: "14px 15px",
                            borderRadius: 15,
                            background: "rgba(255,178,92,0.07)",
                            border: "1px solid rgba(255,178,92,0.16)",
                        }}
                    >
                        <div
                            style={{
...
function RitualPlayerNav({
    ...
    const navButton = (enabled = true): React.CSSProperties => ({
        ...
        background: enabled
            ? "linear-gradient(180deg, rgba(19,35,44,0.97), rgba(7,15,20,0.99))"
            : "rgba(255,255,255,0.025)",
        color: enabled ? "rgba(232,248,255,0.98)" : "rgba(205,220,228,0.24)",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        padding: 0,
        ...
        boxShadow: enabled
            ? "0 8px 22px rgba(0,0,0,0.28), 0 0 18px rgba(112,211,255,0.09)"
            : "none",
        ...
        >
            ...
            <style>{`
                ...
                * { -webkit-font-smoothing: antialiased; text-rendering: geometricPrecision; }
                ...
                @keyframes tsNextGlow {
                    0%, 100% { filter: brightness(1); }
                    50% { filter: brightness(1.06); }
                }
                .ts-next-best:not(:disabled) { animation: tsNextGlow 2.2s ease-in-out infinite; }
                    ...
                    >
                        <div
                            ...
                            style={{
                                ...
                                padding: "8px clamp(8px, 2vw, 20px) 26px",
                                boxSizing: "border-box",
                            }}
                        >
                            {tab === "saved" ? (
