// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v58.20.7-no-tips FINAL NO-TIPS RELEASE LOCK + ROUTER v1900 + OVERLAY FIT)
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    const recoverSection = (label: string) => {
        ...
        const labels =
            "GOAL|RITUAL|STEPS|WIN|MIND BEND|FORMULA FLOW|SAFETY|PAUSE|RETURN|LOCK IT IN"
        const re = new RegExp(
            `(?:^|\n)${label}\s*\n([\s\S]*?)(?=\n(?:${labels})\s*(?:\n|$)|$)`,
            "i"
        )
