// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (v38.4.3 + v58.19 FRAMER/GITHUB COMPAT FIX)
        ...
        const normalizedBubbles: ManifestBubble[] = raw.bubbles
            .map((entry: any) => {
                if (Array.isArray(entry?.parts) && entry.parts.length) {
                    return {
                        bubble: String(entry?.bubble || entry?.name || "").toUpperCase() as BubbleKey,
                        parts: entry.parts,
                    }
                }
                const bubble = String(
                    entry?.bubble || entry?.name || entry?.bubble_key || entry?.bubbleName || ""
...
function ritualTextDefault(r: BubbleRitual) {
    ...
    const stepsRaw = typeof (r as any)?.steps === "string" && String((r as any).steps).trim()
    ...
    const flow = Array.isArray(flowRaw)
        ? flowRaw.map((x: any) => String(x || "").trim()).filter(Boolean).join(" → ")
        : String(flowRaw || "").trim()
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    if (i < raw.length && isAllCapsTitle(raw[i])) {
        ...
        const diag = raw.join(" ")
        if (/fetch failed|fetch exception|manifest|no parts listed|failed to load|json parse failed/i.test(diag)) {
