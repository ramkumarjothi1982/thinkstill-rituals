...
// Generated from ThinkStill_400_Rituals_FINAL_PRODUCT_READY_MASTER.xlsx
// Routing uses Release Routing Trigger / Secondary Routing Trigger / Release Routing Pattern.
// Internal Hook, Goal and Game Move fields are never rendered.

// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (FINAL PRODUCT-READY MASTER)
...
type FlatManifestFile = {
    ...
    // FINAL master consumer fields
    emotional_distress_cue?: string
    if_not_helpful?: string
    breathwork_support?: string
    mindfulness_support?: string
    positive_psychology_support?: string
    spirituality_support?: string
    affirmation_support?: string
    support_stack?: string
...
function ritualTextDefault(r: BubbleRitual) {
    // FINAL PRODUCT-READY render:
    // distress cue → title → routing/play time → goal → numbered ritual
    // → five supports → one Win → try another ritual → Formula Flow.
    // Internal Hook / Game Move / internal Goal are never rendered.
    const distress = String(r?.emotional_distress_cue ?? "").trim()
    const supports = [
        ["🌬 Breathwork", String(r?.breathwork_support ?? "").trim()],
        ["◉ Mindfulness", String(r?.mindfulness_support ?? "").trim()],
        ["✦ Positive Psychology", String(r?.positive_psychology_support ?? "").trim()],
        ["◇ Spirituality", String(r?.spirituality_support ?? "").trim()],
        ["↗ Affirmation", String(r?.affirmation_support ?? "").trim()],
...
function parsePremiumRitual(text: string): ParsedRitual {
    ...
    const parsed: ParsedRitual = {
        intro: [],
        title: "Emotional Support Ritual",
    ...
    for (; i < raw.length; i++) {
        ...
        if (/^(POWER[- ]?UP|POTENCY STACK|SUPPORT STACK)$/i.test(upper)) {
            section = "powerup"
            continue
        }
