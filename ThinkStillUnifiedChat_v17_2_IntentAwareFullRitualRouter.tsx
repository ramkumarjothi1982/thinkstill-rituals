// ===================== PART 1/5 =====================
// ThinkStill — Unified Chat (INTENT-AWARE FULL-RITUAL ROUTER v17.2)
// ✅ Avatar ALWAYS visible (image OR premium fallback orb)
// ✅ Two-level routing: 22 user-facing errors → 63 precise subpatterns
// ✅ GLOBAL 400-ritual no-replay cycle; reset only after complete exhaustion
...
function sanitizeTextForDisplayStrict(text: string) {
    const raw = normalizeText(text || "")
    const cleaned = raw
        .split("\n")
        .map((l) => stripMetaFromLine(l))
...
function ritualTextDefault(r: BubbleRitual) {
    // v17.2 prefers the complete USER RITUALS workbook fields.
    // Legacy text fields remain as a fallback for older libraries.
    const fullSections = [
        String(r?.title ?? r?.name ?? r?.ritual_name ?? "").trim(),
...
function comfortPrefixForMessage(
    ...
    return level === "two"
        ? pickFrom(pack.two, `${msgSeed}|intent2|${intent}|${hash32(rawMessage)}`)
        : pickFrom(pack.one, `${msgSeed}|intent1|${intent}|${hash32(rawMessage)}`)
}
...
function buildDisplayTitleless(opts: {
    ...
    const main = sanitizeTextForDisplayStrict(rawText)
    const safe = sanitizeTextForDisplayStrict(rawSafety)

    const safetyLine = safe
        ? `Safety: ${safe}`
        : "Safety: If you feel unsafe right now, pause and reach out to a trusted person or local emergency services."

    const comfort = comfortPrefixForMessage(
        comfortLevel,
        msgSeed,
        challengeFromUser
    )
    const challengeLine = formatChallengeLineRaw(challengeFromUser)
    const workbookReturn = String((ritual as any)?.return_cue || "").trim()
    const comeback = workbookReturn || returnCue(bubbleKey)

    const combined = [comfort, challengeLine, main, safetyLine, comeback]
        .filter(Boolean)
        .join("\n\n")
...
/* Two-level weighted routing + one global 400-ritual no-replay cycle */
const ROUTER_VERSION = "v172"
...
function normalizeIssueForRouting(message: string) {
    const stop = new Set([
        ...
        "is","am","are","was","were","be","been","being","i","me","my","mine",
        "it","this","that","these","those","feel","feeling","felt","really",
        "very","just","right","now","today","again","keep","keeps","still",
        "have","has","had","do","does","did","can","could","would","should"
    ])
    const tokens = tokenize(message).filter((w) => !stop.has(w)).slice(0, 28).sort()
...
function getAllRituals(library: Record<BubbleKey, BubbleRitual[]>) {
    const out: Array<{ bubbleKey: BubbleKey; ritual: BubbleRitual; sig: string }> = []
    for (const bk of Object.keys(library) as BubbleKey[]) {
        for (const ritual of library[bk] || []) {
            out.push({ bubbleKey: ritualBubbleKeyFromAny(ritual, bk), ritual, sig: ritualSignature(bk, ritual) })
        }
    }
    return out
}
...
function thinkingErrorOf(r: BubbleRitual) {
    return String(r?.thinking_error ?? r?.challenge_type ?? "General pattern").trim()
}
function subpatternOf(r: BubbleRitual) {
    return String(r?.precise_subpattern ?? r?.primary_trigger ?? r?.challenge ?? thinkingErrorOf(r)).trim()
}
...
function routingHaystack(r: BubbleRitual) {
    return [
        thinkingErrorOf(r), subpatternOf(r), r?.pattern_family, r?.primary_trigger,
        r?.secondary_patterns, r?.recognition_cue, r?.match_reason,
        r?.unique_game_move, r?.coverage_tier, r?.challenge, r?.domain,
        r?.name, r?.ritual_name, ...(Array.isArray(r?.keywords) ? r.keywords : []),
    ].filter(Boolean).join(" ")
}
...
function rankParents(message: string, all: ReturnType<typeof getAllRituals>) {
    ...
    return Array.from(groups.entries())
        .map(([parent, rituals]) => ({ parent, score: scoreParent(message, rituals) }))
        .sort((a, b) => b.score - a.score || a.parent.localeCompare(b.parent))
        .map((x) => x.parent)
}

function rankSubpatterns(message: string, parent: string, all: ReturnType<typeof getAllRituals>) {
    const groups = new Map<string, BubbleRitual[]>()
    for (const row of all) {
    ...
    return Array.from(groups.entries())
        ...
        .sort((a, b) => b.score - a.score || a.subpattern.localeCompare(b.subpattern))
        .map((x) => x.subpattern)
}
...
function pickNextRitualByThinkingError(message: string, library: Record<BubbleKey, BubbleRitual[]>) {
    const all = getAllRituals(library).filter((x) => sanitizeTextForDisplayStrict(ritualTextDefault(x.ritual)).length >= 8)
    ...
    if (!route || !Array.isArray(route.rankedParents) || !route.rankedParents.length) {
        route = { rankedParents: rankParents(message, all), parentIndex: 0, createdAt: Date.now(), updatedAt: Date.now() }
    }
    ...
    for (let p = Math.max(0, route.parentIndex); p < route.rankedParents.length && !chosen; p++) {
        const parent = route.rankedParents[p]
        const parentRemaining = all.filter((x) => thinkingErrorOf(x.ritual) === parent && !used.has(x.sig))
        if (!parentRemaining.length) { route.parentIndex = p + 1; continue }

        if (route.activeParent !== parent || !Array.isArray(route.rankedSubpatterns)) {
            route.activeParent = parent
            route.rankedSubpatterns = rankSubpatterns(message, parent, all)
            route.subpatternIndex = 0
        }

        for (let s = Math.max(0, route.subpatternIndex || 0); s < route.rankedSubpatterns.length; s++) {
            const sub = route.rankedSubpatterns[s]
            const available = parentRemaining
                .filter((x) => subpatternOf(x.ritual) === sub)
                .sort((a, b) => scoreRitualWithinSubpattern(message, b.ritual) - scoreRitualWithinSubpattern(message, a.ritual) || String(a.ritual.id || "").localeCompare(String(b.ritual.id || "")))
            const candidate = available.find((x) => x.sig !== lastGlobal) || available[0]
            if (candidate) {
                chosen = candidate; chosenParent = parent; chosenSubpattern = sub
                route.parentIndex = p; route.subpatternIndex = s
                break
            }
    ...
    if (!chosen) {
        const remaining = all.filter((x) => !used.has(x.sig)).sort((a, b) => scoreRitualWithinSubpattern(message, b.ritual) - scoreRitualWithinSubpattern(message, a.ritual))
        chosen = remaining.find((x) => x.sig !== lastGlobal) || remaining[0] || null
        if (chosen) { chosenParent = thinkingErrorOf(chosen.ritual); chosenSubpattern = subpatternOf(chosen.ritual) }
    }
    if (!chosen) { resetGlobalRitualCycle(); return pickNextRitualByThinkingError(message, library) }

    used.add(chosen.sig)
    saveSet(GLOBAL_USED_RITUALS_KEY, used, all.length + 20)
    lsSet(LAST_GLOBAL_RITUAL_KEY, chosen.sig)

    const subStillAvailable = all.some((x) => thinkingErrorOf(x.ritual) === chosenParent && subpatternOf(x.ritual) === chosenSubpattern && !used.has(x.sig))
    if (!subStillAvailable) route.subpatternIndex = (route.subpatternIndex || 0) + 1
    const parentStillAvailable = all.some((x) => thinkingErrorOf(x.ritual) === chosenParent && !used.has(x.sig))
    if (!parentStillAvailable) {
        route.parentIndex += 1; route.activeParent = undefined; route.rankedSubpatterns = undefined; route.subpatternIndex = 0
    }

    route.updatedAt = Date.now(); routes[issueKey] = route; lsSet(ISSUE_ROUTES_KEY, routes)
    return {
        bubbleKey: chosen.bubbleKey, ritual: chosen.ritual, text: ritualTextDefault(chosen.ritual), safety: ritualSafety(chosen.ritual),
        thinkingError: chosenParent, preciseSubpattern: chosenSubpattern,
        cycle: Number(lsGet<number>(GLOBAL_CYCLE_KEY, 1) || 1), usedCount: used.size, totalCount: all.length,
    }
}
...
function bubbleBadgeStyle(
    ...
    return {
        ...
        boxShadow: perf
            ...
            : "0 0 18px rgba(140,220,255,0.10), inset 0 1px 0 rgba(255,255,255,0.07)",
...
export default function ThinkStillUnifiedChat(props: any) {
    ...
    const ritualBaseSize = Math.max(14, Number(ritualFontSize || 18))

    const textCol = "rgba(248,252,255,0.92)"
    const dimCol = "rgba(245,249,255,0.72)"
    ...
    function normalizeLoadedRitual(x: any): BubbleRitual {
        return {
            ...
            thinking_error: String(x?.thinking_error ?? x?.thinkingError ?? x?.trigger ?? "").trim(),
            precise_subpattern: String(x?.precise_subpattern ?? x?.preciseSubpattern ?? x?.primary_trigger ?? "").trim(),
            primary_trigger: String(x?.primary_trigger ?? x?.primaryTrigger ?? "").trim(),
            unique_game_move: String(x?.unique_game_move ?? x?.uniqueGameMove ?? "").trim(),
            coverage_tier: String(x?.coverage_tier ?? x?.coverageTier ?? "").trim(),
            routing_weight: x?.routing_weight ?? x?.routingWeight ?? 0,
            routing_probability_pct: x?.routing_probability_pct ?? x?.routingProbabilityPct ?? 0,
            parent_pool_size: x?.parent_pool_size ?? x?.parentPoolSize ?? 0,
            bubble_pool_size: x?.bubble_pool_size ?? x?.bubblePoolSize ?? 0,
            pattern_family: String(x?.pattern_family ?? x?.patternFamily ?? "").trim(),
            secondary_patterns: String(x?.secondary_patterns ?? x?.secondaryPatterns ?? "").trim(),
            pattern_type: String(x?.pattern_type ?? x?.patternType ?? "").trim(),
            recognition_cue: String(x?.recognition_cue ?? x?.recognitionCue ?? "").trim(),
            situation: String(x?.situation ?? "").trim(),
            immediate_need: String(x?.immediate_need ?? x?.immediateNeed ?? "").trim(),
            keywords: Array.isArray(x?.keywords) ? x.keywords.map((v: any) => String(v || "").trim()).filter(Boolean) : [],
            example_entry: String(x?.example_entry ?? x?.exampleEntry ?? "").trim(),
            match_reason: String(x?.match_reason ?? x?.matchReason ?? "").trim(),
    ...
    // Scroll ref (auto-scroll on new ritual)
    ...
    React.useEffect(() => {
        ...
        if (!el) return
        // Let layout settle, then smooth scroll down.
        const t = setTimeout(() => {
            try {
    ...
    const sendMessage = async (overrideMessage = "", fromMic = false) => {
        markGesture()
        ...
        try {
            ...
            // Thinking-error ranking needs the full 400-ritual pool.
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            Automatic Thinking-Error Route
                        </span>
                ...
                >
                    ...
                    >
                        ...
                        >
                            <span style={{ opacity: 0.85 }}>
                                ...
                                libProgress.total ? (
                                    ...
                                    >
                                        {libProgress.loaded}/{libProgress.total}
                                    </span>
                                ) : null}
                            </span>
                        ...
                        >
                            ...
                            <button
                                className={`ts-haptic ${burstClear.burstKey ? "" : ""}`}
                                onPointerDown={() => burstClear.fire()}
                                onClick={() => clearSaved()}
                                title="Clear Saved"
                                disabled={!saved.length}
                                style={miniBtn("danger", uiFontFamily)}
                            >
                                ⟲ Clear
                            </button>
                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            <AnimatedText
                                                ...
                                                runKey={runKey}
                                                msPerChar={effectiveMsPerChar}
                                                rgbIntensity={rgbI}
                                                rgbSpeedFactor={rgbSpeed}
                                                performanceMode={
                                                    !!performanceMode
                                                }
                                                initialChars={currentTypedChars}
                                                onProgress={(chars) => {
                                                    markTypingProgress(
                                                        currentSig,
                                                        chars
